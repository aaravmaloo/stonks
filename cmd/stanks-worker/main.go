...
func main() {
	...
	if runOnce {
		if err := svc.RunMarketTick(ctx, seasonID, cfg.MarketTickEvery, cfg.EmployeePerTick, cfg.InterestAPR, cfg.MarketVolatility); err != nil {
	...
	logger.Info("worker started", "tick_every", cfg.MarketTickEvery.String(), "employee_per_tick", cfg.EmployeePerTick, "volatility", cfg.MarketVolatility)
	for {
		select {
		case <-ctx.Done():
